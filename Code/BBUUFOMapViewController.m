...
@interface BBUUFOMapViewController () <MKMapViewDelegate, UISearchBarDelegate>
...
@implementation BBUUFOMapViewController
...
-(void)searchTapped:(UIBarButtonItem*)item {
    ...
    if (self.searchBar) {
        [UIView animateWithDuration:1.0 animations:^{
            ...
            for (BBUUFOSighting* item in self.items) {
                if (item.title) {
...
-(void)setItems:(NSArray *)items {
    ...
    for (BBUUFOSighting* item in items) {
        if (item.title) {
            [self.mapView addAnnotation:item];
        }
    }
...
-(void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)view {
    ...
    BBUUFOSighting* sighting = (BBUUFOSighting*)view.annotation;
